...
def get_single_z(image, convert_16_to_8bit, scale_image, auto_scale, scale_min, scale_max, z_stack, find_best_z, max_projection, time):
...
def extract_from_lif(filename, inpath, convert_16_to_8bit, scale_image, auto_scale, scale_min, scale_max, z_stack, find_best_z, max_projection, return_composite, channel_order, return_channels, extension):
	...
	for img in new.get_iter_image(): # each image could be mosaic or single image (single z or z stack) with or without timelapse (TileScan or Position in Lif language)
		for k in range(img.dims.t): # each time in timelapse (dimension t in Lif)
			...
			comp = get_single_z(img, convert_16_to_8bit, scale_image, auto_scale, scale_min, scale_max, z_stack, find_best_z, max_projection, time=k)
			if return_channels:
